...
def generate_fallback_title(content_type, material_title, timestamp=None):
    ...
    return fallbacks.get(
        content_type, f"{material_title} - {content_type.title()} ({timestamp})"
    )
...
def gather_material_text(material, specific_attachment_ids=None) -> str:
    ...
    if specific_attachment_ids:
        ...
        print(
            f"🎯 Processing {len(specific_attachment_ids)} specific attachments from material '{material.title}'"
        )
    else:
        attachments = material.attachments.all()
        print(
            f"📁 Processing all {attachments.count()} attachments from material '{material.title}'"
        )
    ...
    for attachment in attachments:
        try:
            ...
            if extracted_text and extracted_text.strip():
                ...
                print(
                    f"✅ Extracted {len(extracted_text)} characters from {attachment.file.name}"
                )
            else:
...
def generate_flashcards_from_material(
    material, num_cards: int = 5, specific_attachment_ids=None
) -> dict:
    ...
    if not text_body:
        if specific_attachment_ids:
            ...
            raise ValueError(
                "No extractable text found in this Material's attachments."
            )
    ...
    try:
        ...
        try:
            ...
            raise ValueError(
                f"AI did not return valid JSON for flashcards: {e}\n\nRaw response: {raw}"
            )
        ...
        for idx, obj in enumerate(cards):
            ...
            if not q or not a:
                raise ValueError(
                    f"Flashcard #{idx+1} missing question or answer: {obj}"
                )
            validated.append({"question": q, "answer": a})

        # ✅ Validate and improve title
        improved_title = validate_and_improve_title(
            title.strip(), "flashcards", material.title
        )
...
def generate_notes_from_material(material, specific_attachment_ids=None) -> dict:
    ...
    try:
        ...
        try:
            ...
            print(f"❌ JSON parsing failed. Raw response: {raw}")
            raise ValueError(
                f"AI did not return valid JSON for notes: {e}\n\nRaw response: {raw}"
            )
        ...
        # ✅ Validate and improve title
        improved_title = validate_and_improve_title(
            title.strip(), "notes", material.title
        )
...
def generate_quiz_from_material(
    material, num_questions: int = 5, specific_attachment_ids=None
) -> dict:
    ...
    try:
        ...
        try:
            ...
            raise ValueError(
                f"AI did not return valid JSON for quiz: {e}\n\nRaw response: {raw}"
            )
        ...
        print(
            f"🔍 Quiz validation - Title: {title}, Description: {description}, Questions count: {len(questions) if questions else 0}"
        )
        ...
        for idx, qobj in enumerate(questions):
            try:
                ...
                print(
                    f"🔍 Question #{idx+1} - Text: '{qt[:50]}...', Choices: {len(ch) if ch else 0}, Correct: '{ca[:50]}...'"
                )
                ...
                if len(ch) < 2:
                    raise ValueError(
                        f"Question #{idx+1} needs at least 2 choices, got: {len(ch)}"
                    )
                ...
                for i, choice in enumerate(ch):
                    if not isinstance(choice, str):
                        raise ValueError(
                            f"Question #{idx+1} choice #{i+1} is not a string: {choice}"
                        )
                ...
                if ca not in cleaned_choices:
                    ...
                    matches = [
                        ...
                        if choice.lower().strip() == ca_lower
                    ]
                    if matches:
                        ...
                        for i, choice in enumerate(cleaned_choices):
                            print(f"   {i+1}. {choice}")
                        raise ValueError(
                            f"Question #{idx+1} correct_answer '{ca}' not found in choices"
                        )
                ...
                raise ValueError(
                    f"Question #{idx+1} validation failed: {validation_error}"
                )
        ...
        # ✅ Validate and improve title
        improved_title = validate_and_improve_title(
            title.strip(), "quiz", material.title
        )
    ...
    if conversation.should_regenerate_summary():
        try:
            ...
            new_summary = generate_conversation_summary(
                messages_to_summarize, conversation.summary_context
            )
